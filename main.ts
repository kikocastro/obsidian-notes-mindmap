...
import {
  MapCfg, MNode, NoteLike, Resolver,
  catColor, collectNodes, buildEdges, isSecondary, fieldArr,
  computeVisible, orderAndLayout, searchMatch, wrap, validateConfig,
} from "./src/graph";
...
interface LinkRow { id: string; title: string; levelLabel: string; color: string; secondary: boolean; relation: "parent" | "child"; }
...
function renderMindmap(app: App, plugin: Plugin, source: string, host: HTMLElement, ctx: MarkdownPostProcessorContext) {
  ...
  const notes: NoteLike[] = app.vault.getMarkdownFiles().map((f) => {
    fileByPath[f.path] = f;
    return { path: f.path, basename: f.basename, frontmatter: app.metadataCache.getFileCache(f)?.frontmatter || {} };
  });
  const resolveLink: Resolver = (key, fromPath) => app.metadataCache.getFirstLinkpathDest(key, fromPath)?.path ?? null;
  ...
  resetBtn.onclick = () => {
    ...
    toolbar.querySelectorAll(".mm-chip.on").forEach((c) => (c as HTMLElement).removeClass("on"));
    draw(); fit();
  };
  ...
  function linksFor(n: MNode): LinkRow[] {
    const row = (id: string, relation: "parent" | "child"): LinkRow => {
      ...
      return { id, title: o.title, levelLabel: o.levelLabel, color: o.color, secondary: sec, relation };
    };
    return [...[...n.parents].map((p) => row(p, "parent")), ...[...n.children].map((c) => row(c, "child"))];
  }
  function openNode(id: string) {
    selected = id; reapply();
    const n = nodes[id];
    new NoteModal(app, plugin, n, linksFor(n), fileByPath[n.path], openNode).open();
  }
  ...
  function draw() {
    ...
    const { order, levelX, contentBottom: cb, contentRight: cr } = orderAndLayout(cfg, nodes, byLevel, vis);
    contentBottom = cb; contentRight = cr;
...
class NoteModal extends Modal {
  ...
  constructor(
    app: App, private plugin: Plugin, private node: MNode,
    private links: LinkRow[], private file: TFile, private nav: (id: string) => void,
  ) { super(app); }
  ...
  async onOpen() {
    ...
    if (n.basename && n.basename !== n.title) head.createDiv({ cls: "mm-note-file", text: n.basename });
    ...
    open.onclick = () => { this.app.workspace.getLeaf("tab").openFile(this.file); this.close(); };
    ...
    await MarkdownRenderer.render(this.app, content, body, this.file.path, this.comp);
  }

  private renderLinks(contentEl: HTMLElement) {
    if (!this.links.length) return;
    const wrapEl = contentEl.createDiv({ cls: "mm-note-links" });
    ([["Parents", "parent"], ["Children", "child"]] as const).forEach(([label, rel]) => {
      const rows = this.links.filter((l) => l.relation === rel);
      if (!rows.length) return;
      const grp = wrapEl.createDiv({ cls: "mm-note-linkgroup" });
      grp.createDiv({ cls: "mm-note-linklabel", text: `${label} (${rows.length})` });
      rows.forEach((l) => {
        const btn = grp.createEl("button", { cls: "mm-link-row" + (l.secondary ? " mm-link-sec" : "") });
        ...
        btn.onclick = () => { this.close(); this.nav(l.id); };
      });
    });
  }
