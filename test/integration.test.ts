...
const notes = [
  ...
  mk("strategy/kpis/K1.md", {
    title: "K1",
    north_star: "[[NS1]]",
    metric: "m",
    horizon: "now",
  }),
  mk("strategy/kpis/K2.md", {
    title: "K2",
    north_star: "[[NS1]]",
    metric: "m",
    horizon: "next",
  ...
  mk("strategy/drivers/D1.md", {
    title: "D1",
    business_kpi: "[[K1]]",
    metric: "m",
  }),
  mk("strategy/drivers/D2.md", {
    title: "D2",
    business_kpi: "[[K2]]",
    metric: "m",
    serves: ["[[O3]]"],
  ...
  mk("strategy/opps/O1.md", {
    title: "O1",
    "ladders-to": "[[D1]]",
    kind: "growth",
    horizon: "now",
    demand: ["A (client)", "B (client)", "C (prospect)"],
  }),
  mk("strategy/opps/O2.md", {
    title: "O2",
    "ladders-to": "[[D1]]",
    kind: "retention",
    horizon: "next",
  ...
  mk("strategy/roadmap/R1.md", {
    title: "R1",
    customFields: { serves: ["[[O1]]"] },
    status: "active",
    progress: 60,
    parentId: null,
  }),
  mk("strategy/roadmap/R2.md", {
    title: "R2",
    customFields: { serves: ["[[O2]]"], alsoServes: ["[[O1]]"] },
    status: "done",
    progress: 100,
  }),
  mk("strategy/roadmap/R3.md", {
    title: "R3",
    customFields: { serves: ["[[O3]]"] },
    status: "active",
    progress: 0,
    parentId: "",
  }),
  mk("strategy/roadmap/R4.md", {
    title: "R4",
    customFields: { serves: ["[[O1]]"] },
    status: "active",
    parentId: "R1",
...
const cfg: MapCfg = {
  levels: [
    {
      id: "northstar",
      from: "strategy/north-star",
      card: { title: "title", sub: "metric" },
    },
    {
      id: "kpis",
      from: "strategy/kpis",
      card: { title: "title", sub: "metric", meta: ["horizon"] },
    },
    {
      id: "drivers",
      from: "strategy/drivers",
      card: { title: "title", sub: "metric" },
    },
    {
      id: "opportunities",
      from: "strategy/opps",
      card: { title: "title", meta: ["kind", "horizon"], bars: "demand" },
    },
    {
      id: "roadmap",
      from: "strategy/roadmap",
      where: { parentId: null },
...
describe("OST integration (oracle cross-check)", () => {
  ...
  it("builds primary edges across all levels, with one secondary (dashed) cross-link", () => {
    ...
    expect(
      isSecondary(edgeKind, "strategy/opps/O1.md", "strategy/roadmap/R2.md")
    ).toBe(true);
    expect(
      isSecondary(edgeKind, "strategy/opps/O2.md", "strategy/roadmap/R2.md")
    ).toBe(false);
  });

  it("reverse + dotted via resolve their edges", () => {
    expect(nodes["strategy/opps/O3.md"].primaryParent).toBe(
      "strategy/drivers/D2.md"
    ); // via reverse serves
    expect(nodes["strategy/roadmap/R1.md"].primaryParent).toBe(
      "strategy/opps/O1.md"
    ); // via customFields.serves
  });

  it("every non-root node has a primary parent (no orphans in this fixture)", () => {
    const orphans = Object.values(nodes).filter(
      (n) => n.levelIdx > 0 && !n.primaryParent
    );
  ...
  it("extracts bars and numeric progress (including 0)", () => {
    expect(nodes["strategy/opps/O1.md"].bars).toEqual([
      ["client", 2, CATEGORY_COLORS.client],
  ...
  it("lays out all visible nodes without dropping any", () => {
    ...
    const { order, contentRight, contentBottom } = orderAndLayout(
      cfg,
      nodes,
      byLevel,
      vis
    ...
    expect(nodes["strategy/kpis/K1.md"].x!).toBeGreaterThan(
      nodes["strategy/north-star/NS1.md"].x!
    );
  });
});
